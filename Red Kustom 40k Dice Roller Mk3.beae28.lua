...
printDiceChancesBtn={
    ...
    position={2.5,0.1,0.0}, rotation={0,0,0}, height=50, width=400,
    font_size=60, color={0,0,0}, font_color={1,1,1}, scale={scaleBtn,scaleBtn,scaleBtn}
}
...
function printResults(n)
        ...
        if n==1 then
            		printToAll("LAST 1 ROLL")
        elseif n>=#lastRolls then
            		printToAll("LAST "..#lastRolls.." ROLLS")
        else
            		printToAll("LAST "..n.." ROLLS")
        end
    	end
    	...
    	for i, result in ipairs(lastRolls) do
        		if i<= n then
            			...
            			printToAll("\n#"..i.." "..string.sub(result.msg,3),result.color)
        		end
    	end
end
...
function setValueCallback(obj, tab)
    function insidef()
        ...
        local rot = objectToPlaceDiceOn.getRotation()
        local r = obj.getRotation()
        local v = obj.getValue()

        if data.RotateDice == 'yes' then
            if callFaces == 4 then
